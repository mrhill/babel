...
#define bbDECLAREARR(T,name,elsize) struct name##Rec {\
    ...
    inline void      Del(const bbUINT index, const bbUINT count) { bbMemMove(mpData+index, mpData+index+count, (mSize-index-count)*elsize); Grow(-(int)count); }\
...
bbERR bbArr_SetSize56(void* const p, const bbUINT size);
...
void* bbArr_Grow56(void* const p, const int relsize);
...
bbERR bbArr_CopyFrom56(void* const p, void* const pFrom);
